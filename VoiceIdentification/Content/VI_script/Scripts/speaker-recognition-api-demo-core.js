...
function startListeningForIdentification(){
   ...
   // console.log(profileIds);
    //console.log(profileIds.toString());
    profileidsString=profileIds.toString();
	if (profileIds.length > 0 ){
	    //console.log('I\'m listening... just start talking for a few seconds...');
	    $("#process").text("I\'m listening... just start talking for a few seconds...");
	//	console.log('Maybe read this: \n' + thingsToRead[Math.floor(Math.random() * thingsToRead.length)]);
...
//	var Ids = profileIds.map(x => x.profileId).join();
	...
	request.onload = function () {
	    ...
	    $("#process").text("identifying profile");
	...
	enrolledInterval = setInterval(function()
	{
		...
		request.onload = function()
		{
			...
			if (json.status == 'succeeded' && json.processingResult.enrollmentStatus == 'Enrolled')
			{
			    ...
			    $("#process").text("Enrollment Complete");
				console.log('enrollment complete!');
				//ar name = window.prompt('Who was that talking?');
				//profileIds.push(new Profile(profileId));
				//console.log(profileIds.length);
			    //console.log(profileId + ' is now mapped to ' + name);
			    // call to store it in db
...
function BurnItAll(mode = 'identification'){
    ...
    //var key = qs['key'];
    var key ='c3f0f950f07c417f9e241db95fcfb424';
        ...
        $.ajax({
           ...
           .done(function(data) {
              // alert("success"+ JSON.stringify(data.length));
               ...
               console.log(profileIds.length +"from getall profile");
